...
const titles = [
  'О моем 5-ти летнем опыте',
  'Мои занятия - какие они?',
  'Мое знакомство с китайским',
  'Как я обучалась'
]
...
const showBiographyBlock = async (inx: number) => {
  ...
  abortPreviousAnimation = () => { currentCallStatus.aborted = true }
...
const debounce = ( params: { cb: ( ...args: any[] ) => void, time: number }) => {
  ...
  return ( ...args: any[] ) => {
    ...
    timeout = setTimeout(() => { params.cb(...args) }, params.time);
  }
}
