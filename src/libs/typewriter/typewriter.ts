...
export class TypeWritterHtml implements ITypeWriterTarget {
  ...
  pop() {
    ...
    if (!this.$el.lastChild) {
      throw new Error(
        "Attemption of removing char when $el have been already emptied"
      );
    }

    const $newRemovedEl = (this._$removedChar
      ? this._$removedChar.previousSibling
      : this.$el.lastChild) as HTMLElement | null;
...
export class TypeWriter {
  ...
  private _resolveWriting:
    | null
    | ((value: EWritingProcessResults) => void) = null;

  constructor(target: ITypeWriterTarget) {
    this._target = target;
  }

  private _getClearMutationParams(
    params: ITextMutationParams
  ): Required<ITextMutationParams> {
    return {
  ...
  private _runTextMutationIteration(
    acc: string,
    params: ITextMutationIterationParams,
    promiseStatusContainer: { status: EWritingProcessResults | null },
  ) {
    if (!this._resolveWriting) {
      throw new Error(
        'Function "resolve" was not provided for resolving text mutation'
      );
    }
  ...
  writeText(
    v: string,
    params: ITextMutationParams
  ): Promise<EWritingProcessResults> {
    ...
    const {
      ...
      insertationTime,
      timeAmplitude
    } = this._getClearMutationParams(params);
