...
class FileMetricsPrinter extends MetricsPrinter {
  ...
  private _prepareMetrics( metricsPerFiles: IMetricsPerFile[] ): IMetricOutput[] {
    const outputs: IMetricOutput[] = structuredClone(metricsPerFiles).flat().map( (metricsPerFile, inx, arr ) => ({
      titleSize: [
    ...
    normilizeLength(
      outputs,
      'titleSize',
      ( [ prefix, title, size ], mod ) => [ prefix, title, mod, size ],
    )

    normilizeLength(
      outputs,
      'topic',
      ( [ topic ], mod ) => topic === '' ? ['', ''] : [ topic, mod ],
      0
    )
    ...
    normilizeLength(
      outputs,
      'percent',
      ( [ percent ], mod ) => percent === '' ? ['', ''] : [ percent, mod ],
      0
    )
  ...
  printMetricPerTopic( ...[ topic, metricPerTopic ]: Parameters<InstanceType<typeof MetricsPrinter>['printMetricPerTopic']> ) {
    ...
    normilizeLength(
      output,
      'output',
      ( [ label, value ], mod ) => [ label, mod, value ],
      2
    )

    const logWithBullet = (last: boolean, ...args: string[] ) => console.log(
      colorPalette.muted(this._getBulletByLast(false, last)) + args[0],
      ...args.slice(1)
    )
    console.log('')
...
export class FileTransformationMetrics extends ProductivityMertrics {
  ...
  record( file: IFile<Buffer>, transformedFiles: ITransformedFile<Uint8Array>[]) {
    ...
    const transformedFilesMetrics = transformedFiles.map(
      file => metricToTransformingMetric( originalFileMetric.size, {
        size: file.content.byteLength,
