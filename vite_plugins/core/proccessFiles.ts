...
import { FileSystemManipulator } from './FileSystemManipulator'
...
export const proccessFiles = async ( opts: IProccessFilesOpts ) => {
  try {
    ...
    fileTransformationMetrics.addTopics(opts.transforms.map( transform => ({
      topic: transform.name
    }) ))

    const allFilePaths = await FileSystemManipulator.getFilesPaths(opts.patterns, opts.ignores)
    ...
    const readedFiles = await FileSystemManipulator.readFiles( nonGeneratedFilePaths )

    for(const file of readedFiles) {
      const transformedFiles = await transformer.transform( file )
      transformedFiles.forEach( file => FileSystemManipulator.writeFile( file ) )
